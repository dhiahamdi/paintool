  ...
  <div class="container">
    <div class="row">
        ...
        <div class="col-3 offset-6 text-center float-end">
          <router-link  class="btn btn-danger btn-lg" to="/front">Front view</router-link>
        </div>
      </div>
      <br>
    <div class="row">
...
export default {
  ...
  methods: {
    get(key) {
      ...
      this.image = store.state.img_back
    },
    set(key) {
      var vm = this;
      try {
        //localStorage.setItem(key, JSON.stringify(this.image));
        store.setImgBack(this.image)
      } catch (e) {
  ...
  mounted() {
    ...
    if(store.state.layer_back == null){
      this.layer = new Konva.Layer();
    }
    else{
      this.layer = store.state.layer_back;
    }
